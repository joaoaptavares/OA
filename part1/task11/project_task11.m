...
waypoints_captured(1) = waypoints_captured_0;
...
grid on;
axis([0 35 -15 15]);
saveas(plt_x, sprintf('task11_pltx_%d.png', m));
...
saveas(plt_u, sprintf('task11_pltu_%d.png', m));
    

    for j = 1:K
        if norm(E*x(:,time(j))- c_k(:,j)) <= 10^(-6)
            waypoints_captured(m+1) = waypoints_captured(m+1) + 1;
        end    
    end
